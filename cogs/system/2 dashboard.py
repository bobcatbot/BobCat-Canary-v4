    ...
    def __init__(self, guild: discord.Guild):
        ...
        class VerificationSelect(ActionRow):
            @select(
                placeholder="Select an option",
                options=[
                    discord.SelectOption(label="Verification Message"),
                ],
            )
            async def callback(self, select, interaction: discord.Interaction):
                if select.values[0] == "Verification Message":
                    await interaction.response.send_message("You selected Verification Message")
