...
async def form_submissions(guild_id, form_id):
    try:
        ...
        logger.error(f"Error loading submissions for form {form_id} in guild {guild_id}: {e}", exc_info=True)
        await flash('An error occurred loading submissions', 'error')
        return redirect(url_for('web.index'))
...
async def form_submission_detail(guild_id, form_id, submission_id):
    try:
        ...
        logger.error(f"Error loading submission {submission_id} for form {form_id}: {e}", exc_info=True)
...
async def form_submission_delete(guild_id, form_id, submission_id):
    try:
        ...
        logger.error(f"Error deleting submission {submission_id} for form {form_id}: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500
