...
async def stats(guild_id):
    try:
        ...
        logger.error(f"Error loading stats page for guild {guild_id}: {e}", exc_info=True)
...
async def stats_setup(guild_id):
    ...
    try:
        ...
        logger.error(f"Error in stats_setup route: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
async def stats_refresh(guild_id):
    ...
    try:
        ...
        logger.error(f"Error in stats_refresh route: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
async def stats_create_counter(guild_id):
    ...
    try:
        ...
        return jsonify({'status': 'error', 'message': 'No permissions to create voice channel'}), 403
    except Exception as e:
        logger.error(f"Error in stats_create_counter: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
async def stats_delete_counter(guild_id, counter_idx):
    ...
    try:
        ...
        logger.error(f"Error in stats_delete_counter: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
async def stats_reset(guild_id):
    ...
    try:
        ...
        logger.error(f"Error in stats_reset: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
async def stats_reorder(guild_id):
    ...
    try:
        ...
        logger.error(f"Error in stats_reorder: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500