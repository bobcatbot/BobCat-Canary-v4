...
async def ticketing_transcript(guild_id, ticket_id):
    try:
        ...
        logger.error(f"Error loading ticket transcript for guild {guild_id}: {e}", exc_info=True)
        await flash('An error occurred loading the ticket', 'error')
        return redirect(url_for('web.index'))
...
async def ticketing(guild_id):
    try:
        ...
        logger.error(f"Error loading ticketing page for guild {guild_id}: {e}", exc_info=True)
...
async def ticketing_create(guild_id):
    try:
        ...
        logger.error(f"Error in ticketing_create route for guild {guild_id}: {e}", exc_info=True)
...
async def ticketing_edit(guild_id, ticket_id):
    try:
        ...
        logger.error(f"Error in ticketing_edit route for guild {guild_id}: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
async def ticketing_delete(guild_id, ticket_id):
    try:
        ...
        logger.error(f"Error in ticketing_delete route for guild {guild_id}: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500