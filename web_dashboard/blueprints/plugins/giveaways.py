...
async def giveaways(guild_id):
    try:
        ...
        logger.error(f"Error loading giveaways page for guild {guild_id}: {e}", exc_info=True)
...
async def giveaways_creation(guild_id):
    try:
        ...
        logger.error(f"Error in giveaways_creation route for guild {guild_id}: {e}", exc_info=True)
...
async def giveaways_edition(guild_id, gway_id):
    try:
        ...
        logger.error(f"Error in giveaways_edition route for guild {guild_id}: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500