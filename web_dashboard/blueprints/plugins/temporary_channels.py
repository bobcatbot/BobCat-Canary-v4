...
async def temporary_channels(guild_id):
    try:
        ...
        logger.error(f"Error loading temporary channels page for guild {guild_id}: {e}", exc_info=True)
...
async def temporary_channels_create(guild_id):
    try:
        ...
        logger.error(f"Error in temporary_channels_create route for guild {guild_id}: {e}", exc_info=True)
...
async def temporary_channels_edit(guild_id, hub_id):
    try:
        ...
        logger.error(f"Error in temporary_channels_edit route for guild {guild_id}: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
async def temporary_channels_delete(guild_id, hub_id):
    try:
        ...
        logger.error(f"Error in temporary_channels_delete route for guild {guild_id}: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500