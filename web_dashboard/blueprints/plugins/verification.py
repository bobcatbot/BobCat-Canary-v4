...
async def verify(guild_id):
    try:
        ...
        logger.error(f"Error loading verification page for guild {guild_id}: {e}", exc_info=True)
...
async def verify_publish(guild_id):
    ...
    try:
        ...
        # Fire and forget - instant response to user!
        asyncio.create_task(publish())
        
        return jsonify({'status': 'success', 'message': 'Publishing verification message in background...'})
    
    except Exception as e:
        logger.error(f"Error in verify_publish route for guild {guild_id}: {e}", exc_info=True)
...
async def verify_unpublish(guild_id):
    ...
    try:
        ...
        logger.error(f"Error in verify_unpublish route for guild {guild_id}: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
async def verify_update(guild_id):
    ...
    try:
        ...
        logger.error(f"Error in verify_update route for guild {guild_id}: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500